\section{Background}
This section introduces the necessary notations and definitions. To simplify the presentation, the MPI standard is used in the following discussion. The algorithm for deadlock detection is further adaptable to other message passing standards. In an MPI program, it is assumed that only non-blocking sends and receives are used. 
%Also, this paper considers only single-path MPI programs in the following discussion. 
