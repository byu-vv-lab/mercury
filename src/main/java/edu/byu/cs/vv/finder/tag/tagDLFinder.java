...
import edu.byu.cs.vv.match_generation;
...
public class tagDLFinder extends AbstractFinder {
    
    private MatchPairGenerator matchGenerator;
    
    public tagDLFinder(Program program) {
        super(program);
        matchGenerator = new MatchPairGenerator();
    }
    
    @Override
    public boolean verify() {
        Set<UnmatchedTagPattern> patterns = generatePatterns(program);
        if (patterns.isEmpty()) {
            log = "No unmatched tag patterns found";
            return (result = true);
        }
        for (UnmatchedTagPattern pattern : patterns) {
            ...
            while (schedulable(stepper, pattern, sendNums, recvNums, witnessedRecv)) {
                ...
                if (stepper.atBlockPoint()) {
                    ...
                    if (reachableRanks.isEmpty()) {
                        ...
                        //??????
                        AbstractEncoder encoder = new tagEncoder(stepper,
                                                                  pattern,
                                                                  lastrInShape,
                                                                  lastsInShape,
    ...
    /**
     * Returns a set of all process' UnmatchedTag patterns
     *
     * @param prog
     * @return
     */
    private static Set<UnmatchedTagPattern> generatePatterns(Program prog) {
        Set<UnmatchedTagPattern> set = new HashSet<>();
        for (Process process : prog) {
            // generate patterns if tag deterministic receives exist
            if (process.hasTagDeterminsticRecv()) {
     ...
     * @return
     */
    private static Set<UnmatchedTagPattern> generatePatterns(Process proc) {
        Set<UnmatchedTagPattern> set = new HashSet<>();
        boolean hasPred = false;
        for (Receive recv : proc.getReceiveList()) {
            if (recv.isTagWildcard()) {
                hasPred = true;
            } else if (hasPred) {
                set.add(new UnmatchedTagPattern(recv, proc));
            }
        }
        return set;
    }
