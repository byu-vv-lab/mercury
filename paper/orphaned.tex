...
\end{lemma}
\begin{proof}
\algoref{algo:mismatch} considers all possible pairs of wildcard/deterministic receives on each process. Also, the algorithm only removes the pairs that do not satisfy the criteria of \defref{def:mismatch}. Therefore, all possible instances for the orphaned receive pattern are detected. 
\end{proof}
...
\end{lemma}
\begin{proof}
The soundness and completeness relies on the correctness proof of the SMT encoding \cite{DBLP:conf/kbse/HuangMM13}. In general, a satisfying assignment returned represents a feasible schedule leading to an orphaned receive. Therefore, a real deadlock is detected. If the SMT encoding is unsatisfiable indicating that no schedules can be resolved for $pt$, then no real deadlock may occur.
\end{proof}
