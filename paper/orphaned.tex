...
The SMT encoding used in the validation step for the orphaned receive pattern can be solved by a state-of-art SMT solver with a higher cost \cite{}. The general algorithm, however, can be modified to remove the validation for the orphaned receive pattern, which leads to a much faster deadlock detection. If so, the algorithm is not sound but is complete.
